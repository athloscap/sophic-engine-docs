...
The Sophic Engine API uses HTTP response codes to indicate the success or failure of a
request. Coees in the `200-299` range indicate success, codes in the `400-499` range indicate
a client error (e.g failed validation) and should be fixed and retried in your end, and
codes in the `500-599` range indicate a server error on our end. Some endpoints are
...
headers for a unique request ID (`X-Request-Id`). This ID is assigned as soon as the
...
`X-Client-Request-Id` header. This header is not added automatically; you must explicitly
set it on the request.

The format is up to you; we recommend using a UUID or your internal trace ID up to a
maximum of 100 characters. We will reject any request with a `X-Client-Request-Id` header
that is longer than 100 characters.
...
when you can't get the `X-Request-Id` response header, you can share the `X-Client-Request-Id`
