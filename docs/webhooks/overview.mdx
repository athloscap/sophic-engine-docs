...
To get started, use the [Create webhook](/api-reference/create-a-webhook) endpoint to register a webhook URL.
...
| `Webhook-Id` | The unique ID of this delivery (e.g. `whd_abc123`). Use this to [deduplicate](/documentation/webhooks/deliveries#handling-duplicates) retried deliveries. |
| `Webhook-Timestamp` | Unix timestamp (seconds) of when the delivery was dispatched. Use this to [reject stale messages](/documentation/webhooks/verifying-signatures#rejecting-stale-webhooks). |
| `Webhook-Signature` | HMAC-SHA256 signature(s) of the payload. Multiple signatures may be present during [secret rotation](/documentation/webhooks/verifying-signatures#secret-rotation). |
...
When an event occurs, we send a `POST` request to your registered URL. The webhook payload follows the same schema as the [Event](/api-reference/events/get-events) resource:
...
Your endpoint should return a `2xx` status code within **15 seconds** to acknowledge receipt. Any other status code, timeout, or connection error is treated as a failed delivery and will be [retried](/documentation/webhooks/deliveries#retry-schedule). The maximum time we'll wait for a response is 25 seconds (5s connect + 15s read + 5s write).
...
You can use the [Test webhook](/api-reference/test-webhook-delivery) endpoint to send a test delivery to your URL during development. You specify the event `name` and `data` in the request body, and we'll dispatch a delivery to your webhook endpoint. Test deliveries don't include the `Event-Id` header since no real event is created.
